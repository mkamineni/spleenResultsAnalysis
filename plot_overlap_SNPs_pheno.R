...
plot = ggplot(df, aes(x=SNP, y=variable, fill=value))+geom_tile()+labs(x="SNP",y="CAD Risk Factor", size=30)+
theme(axis.text.y = element_text(angle=0, size=30), axis.text.x=element_text(angle=-90, size=30, face="italic"), axis.title=element_text(size=40,face="bold"))+
scale_y_discrete(labels = wrap_10(df$variable))+scale_fill_gradient2(high="#3A68AE")+theme(legend.position="none")
ggsave("../Data/figures/overlap_SNP_figure.pdf", height=20, width = 40, device="pdf")
